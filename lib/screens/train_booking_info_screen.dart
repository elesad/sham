...
import 'train_phone_code_screen.dart';

class TrainBookingInfoScreen extends StatefulWidget {
  ...
  const TrainBookingInfoScreen({Key? key, required this.companyName, required this.fromProvince, required this.toProvince, required this.date, required this.tripNumber, required this.tripTime, required this.price, required this.seatNumber}) : super(key: key);

  @override
  State<TrainBookingInfoScreen> createState() => _TrainBookingInfoScreenState();
}

class _TrainBookingInfoScreenState extends State<TrainBookingInfoScreen> {
  ...
  void _onConfirm() {
    if (_formKey.currentState!.validate() && _birthDate != null && _paymentMethod != null) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => TrainPhoneCodeScreen(bookingInfo: bookingInfo),
        ),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('معلومات الحجز للقطار'),
        centerTitle: true,
                ...
                children: [
                  ...
                  if (_nationality != 'سوريا')
                    Padding(
                      ...
                      child: Icon(Icons.flag, color: Colors.green.shade700),
                    ),
                ],
              ),
